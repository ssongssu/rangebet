...
setTimeout(() => {
    ...
    // Check for click handling on buttons
    console.log('Adding test click listeners to buttons');
    const testMultiplierBtn = document.querySelector('.multiplier-btn[data-multiplier="2"]');
    if (testMultiplierBtn) {
        testMultiplierBtn.addEventListener('click', function() {
            console.log('Test multiplier button clicked directly');
        }, true);
    }

    document.addEventListener('click', function(e) {
        if (e.target.classList.contains('multiplier-btn')) {
            console.log('Multiplier button clicked through document listener', e.target.getAttribute('data-multiplier'));
        }
    }, true);
}, 2000);